...
@ViewScoped
@Named
public class ListaDeMuestreo implements Serializable {
    ...
    public void eliminarMuestreo(Long idMuestreo) throws IOException {
        if (idMuestreo > 0) {
            if (controladorMuestreos.EliminarMuestreo(idMuestreo) > 0) {
                ExternalContext context = FacesContext.getCurrentInstance().getExternalContext();
                context.redirect(context.getRequestContextPath() + "/muestreos/listamuestreos.xhtml?tipomuestra=" + tipoMuestra);
    ...
    public void imprimir(Long id) throws IOException {
        Muestreo m = muestreos.stream()
                .filter(mu -> mu.getId().equals(id))
                .findFirst()
                .orElse(null);
        if (m != null) {
            try {
//                conPdf = new ControladoriTextPdf();
//                ExternalContext context = FacesContext.getCurrentInstance().getExternalContext();
//                conPdf.CrearListadoLaboratorios(context);
//                FacesContext.getCurrentInstance().responseComplete();
//                FacesContext.getCurrentInstance().renderResponse();
            } catch (Exception ex) {
    ...
    public void init() {
        ...
        try {
            ...
            try {
                FacesContext.getCurrentInstance().getExternalContext().redirect("/inlab/errores/error_404.xhtml");

            } catch (IOException ex1) {
                Logger.getLogger(ListaDeMuestreo.class
